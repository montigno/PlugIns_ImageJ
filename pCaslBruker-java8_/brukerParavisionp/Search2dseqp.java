...
public class Search2dseqp {

	private List<String> result = new ArrayList<String>();
	private List<String> listSeq = new ArrayList<String>();
	private List<Integer> noSeq = new ArrayList<Integer>();
	private String rep;
	private String prePdata="";
	private int compt=0;
	
	public Search2dseqp (String rep) {
		this.rep=rep;
		search(new File(rep));
		triSeq();
	};
		    	...
		    	for (File temp : file.listFiles()) {
		    		...
		    		if (temp.isDirectory() && !temp.toString().contains("dicom")) {
		    			...
		    			if (temp.getName().toLowerCase().equals("2dseq") && temp.length()!=0) {
		    				...
		    				noSeq.add(Integer.parseInt(fileFound.substring(rep.length()+1, fileFound.indexOf("pdata")-1))*100+Integer.parseInt(fileFound.substring(fileFound.indexOf("pdata")+6,fileFound.lastIndexOf(File.separator))));

							if (noSeq.size()>1) {
			    				if (fileFound.substring(rep.length()+1, fileFound.indexOf("pdata")-1).contentEquals(prePdata)){
